...
func jumpTrue(mem []int, pos int, modes int) int {
	operand := modeToOperand(modes, mem, pos + 1, 2)
	if *operand[0] != 0 {
		return -pos + *operand[1]
	}
	return 3
}

func jumpFalse(mem []int, pos int, modes int) int {
	operand := modeToOperand(modes, mem, pos + 1, 2)
	if *operand[0] == 0 {
		return -pos + *operand[1]
	}
...
func runProgram(memSlice []int) {
	...
	for instPointer := 0; memSlice[instPointer] != 99; instPointer += incr {
		...
		if opCode == OpAdd {
			...
			incr = jumpTrue(memSlice, instPointer, modes)
		} else if opCode == OpJumpFalse {
			incr = jumpFalse(memSlice, instPointer, modes)
		} else if opCode == OpLessThan {
			incr = applyOp(memSlice, instPointer, modes, lessThan)
		} else if opCode == OpEquals {
			incr = applyOp(memSlice, instPointer, modes, equals)
		} else {
...
func main() {
	...
	prepRunCode("3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9") // In: 0 -> 0
	prepRunCode("3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9") // In: !0 -> 1
	prepRunCode("3,3,1105,-1,9,1101,0,0,12,4,12,99,1") // In: 0 -> 0
	prepRunCode("3,3,1105,-1,9,1101,0,0,12,4,12,99,1") // In: !0 -> 1
	// In: <8 -> 999
	prepRunCode("3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99")
	// In: 8 -> 1000
	prepRunCode("3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99")	// In: 8 -> 1000
	// In: >8 -> 1001
	prepRunCode("3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99")
	...
	prepRunCode(prog) // Pt1 - In: 1
	prepRunCode(prog) // Pt2 - In: 5
}
